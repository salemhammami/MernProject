...
const [marque,setMarque] = useState('')
  ...
  return (
    ...
    {/* <Button type="checkbox" variant="dark"  onClick={toggleFilterVisibility}>
    <FontAwesomeIcon style={{height:'35px'}} icon={faSliders} />{isFilterVisible}
      </Button>
      <div className="FilterClass">
        {isFilterVisible && <Filter filter={filter} setFilter={setFilter} />}
        </div> */}
<Form>
...
<Form.Group controlId="marque">
    ...
    {/* && el.price<=price && el.year == year */}
      {posts &&
        (user.role === "admin", 'client'
          ? 
          // <>
          // {
          //   (marque == 'All' && model == 'All' && title == '')?
          //   <>
          //   {
          //     posts.map((el) => <CardPost key={el._id} el={el} />)
          //   }
          //   </> 
          //   : (marque !== 'All' && model == 'All')?
          //   <>
          //   {
          //     posts.filter(el=>el.marque == marque).map((el) => <CardPost key={el._id} el={el} />)
          //   }
          //   </>
          //   : (marque == 'All' && model !== 'All')?
          //   <>
          //   {
          //     posts.filter(el=>el.model == model).map((el) => <CardPost key={el._id} el={el} />)
          //   }
          //   </>
          //   :
          //   <>
          //   {
          //     posts.filter(el=>el.marque == marque && el.model == model && title != '').map((el) => <CardPost key={el._id} el={el} />)
          //   }
          //   </>
